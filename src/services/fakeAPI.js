...
export const getAllCards = async () => {
  ...
  if (localStorage) {
    let peoplecardstemp = await localStorage.getItem('peoplecards');
    if (peoplecardstemp === null) {
      localStorage.setItem('peoplecards', JSON.stringify(peoplecards));
    } else {
      peoplecards = JSON.parse(peoplecardstemp);
    }
  }
  return peoplecards;
  /*try {
